...
P(2).theta = -pi/8;
P(2).aperture = 64*Trans.spacing; % P(1).aperture in wavelengths
P(2).radius = (P(2).aperture/2)/tan(-P(1).theta); % dist. to virt. apex
...
PData(1).Origin = [-(PData(1).Size(2)/2)*PData(1).PDelta(1),0,P(1).startDepth]; % x,y,z of uppr lft crnr.
...
PData(2).Origin = [-(PData(2).Size(2)/2)*PData(2).PDelta(1),0,P(2).startDepth]; % x,y,z of upper lft crnr.
...
Resource.DisplayWindow(1).Title = 'P4-2vFlashDoppler_32LE_Img_Save';
...
Process(2).method = 'myProcFunction1';
...
Process(3).classname = 'Doppler';                   % process structure for 1st Doppler ensemble
Process(3).method = 'computeCFIFreqEst';
Process(3).Parameters = {'IntBufSrc',[2,1],...          % number of Interbuffer to process.
...
Process(5).method = 'myProcFunction2';
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    Event(n).info = 'Call external Processing function1';
    Event(n).tx = 0;
    Event(n).rcv = 0;
    Event(n).recon = 0;
    Event(n).process = 2;
    ...
    Event(n).info = 'Call external Processing function2';
    Event(n).tx = 0;
    Event(n).rcv = 0;
    Event(n).recon = 0;
    Event(n).process = 5;
...
%save frames
UI(6).Control = {'UserC2','Style','VsSlider','Label','Record',...
                 'SliderMinMaxVal',[0,1,0],'SliderStep',[1,1],'ValueFormat','%3.0f'};
UI(6).Callback = text2cell('%RecordCallback');

EF(1).Function = text2cell('%EF#1'); % external function
EF(2).Function = text2cell('%EF#2'); % external function

% Specify factor for converting sequenceRate to frameRate.
frameRateFactor = 3;

% Save all the structures to a .mat file.
save('MatFiles/P4-2vFlashDoppler_32LE_Img_save');
return
...
%EF#1
myProcFunction1(RData)
persistent myHandle1
P = evalin('base','P');
% Resource = evalin('base','Resource');
% if isempty(myHandle1)||~ishandle(myHandle1)
%     figure;
...
if P(1).record
    imwrite(gray_img, [pwd '/Verasonics_training/Images/Scan/scene',num2str(P.iter),'.png']);
    P(1).iter = P(1).iter+1;
    assignin('base','P', P);
end
% keyboard % debug
...
if isempty(myHandle2)||~ishandle(myHandle2)
    figure;
    myHandle2 = axes('XLim', [0,size(RData,2)], 'YLim', [0, size(RData,1)],...
    'Ydir','reverse','NextPlot', 'replacechildren');
end
P = evalin('base','P');
gray_img = mat2gray(RData);
...
% numcol = numel(unique(gray_img));
imagesc(myHandle2,gray_img);
m = [0,0,0;
    jet(256-1)];
colormap(m);
caxis([0 1]);
...
% 
if P(1).record
    imwrite(gray_img, [pwd '/Verasonics_training/Images/Doppler/doppler',num2str(P.iter),'.png']);
    P(1).iter = P(1).iter+1;
    assignin('base','P', P);
end
% keyboard % debug
