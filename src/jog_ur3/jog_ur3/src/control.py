...
class UR3_control(object):
    def __init__(self):
        ...
        self.docked_flg = 0
        self.init_flg = 0
        self.target_vec = np.zeros((1,2), dytpe = float)
        self.pixl2mm = 0.283
        self.ft_vec = np.zeros((1,3), dytpe = float)
        self.force_goal = np.zeros((1,3), dytpe = float)
        self.force_goal[2] = -5.0

        self.cur_pos = np.zeros((1,3), dytpe = float)
        self.Image_ctrl_pixl = np.zeros((1,2), dytpe = float)
        self.Image_ctrl_pixl[0] = 250
        self.Image_ctrl_pixl[1] = 250
        self.dt = 0.1

        self.error_force_acc =
        self.last_error_force =
        self.last_error_linear
    ...
    def Image_xy_CB(self, msg):
        self.target_vec[0] = msg.data[0] * self.pixl2mm
        self.target_vec[1] = msg.data[1] * self.pixl2mm
    ...
    def rotation_matrix_from_vectors(self, vec1, vec2):
        ...
        rotation_matrix = np.eye(3, dytpe=int) + kmat + kmat*kmat  / (1+c)
    ...
    def Linear_PID(goal, cur, Kp, Ki, Kd, error_in, last_in, dt):
    ...
    def Angular_PID(dif_RPY, Kp, Ki, Kd, error_in, last_in, dt):
        # error_* is the integral historical error
        ...
        angular_X = 0;
        angular_Y = scaler(output[1], 0.1);
        angular_Z = scaler(output[2], 0.1);
        return angular_X, angular_Y, angular_Z, error_out,last_out

    def Force_PID(goal_force, cur_force, Kp, Ki, Kd, error_in, last_in, dt):
    #   stf_factor = [35e4, 35e4, 30e4]; %N/m
        e = goal_force[2] - cur_force[2];
        error_out = error_in + e;
        last_out = e;
        output = Kp .* e  + Ki.* error_out - Kd/dt .* (e - last_in); # PID controller
        linear_fX = scaler(output[0],1);
        return linear_fX, error_out,last_out

    def compare_vel(diff, step):
        if np.abs(diff) > step:
    ...
    def vel_smooth(vel_last, vel_in ,step):
    ...
    def scaler(self, intpu, weight):
        # scale the output in range of [-1, 1]
        input = np. round(input,3) * weight;
        if input >= 1:
            output =1
        elif 0< input && input <=0.05:
            output = 0.05
        elif input < 0 && input >= -0.05:
            output = -0.05
        elif input <=-1:
            output = -1
        else:
    ...
    def controller(self):
        if self.docked_flg:
            [linear_fX, self.error_force_acc,self.last_error_force] = self.Force_PID(self.force_goal, self.ft_vec, 0.05, 0.005, 0, self.error_force_acc, self.last_error_force, self.dt);

            goalMsg = Twist()
            [goalMsg.linear.x, goalMsg.linear.y, goalMsg.linear.z, self.error_linear_acc,self.last_error_linear] = Linear_PID(self.target_vec, np.array([0.0,0.0]), 2, 0, 0.01, self.error_linear_acc, self.last_error_linear, self.dt);
